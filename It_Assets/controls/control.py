...
class Apply:
	def GET(self):
		if not base.logged():
			...
			sysconf = models.getConfig()
			pagesize = sysconf['page']  # page_per_rows
			applycount = models.allApplys()  # apply_count
			para = web.input(page=None,action=None)	
			pages = int(applycount / pagesize)	# page_count

			if (applycount % pagesize):
				pages += 1				#page_count
			...
			if para.action:
				pagerows = models.getApplyPageByLastModify(offset, pagesize)
			else:
				pagerows = models.getApplyPage(offset, pagesize)

			newapplys=[]
			for a in pagerows:
				a['project'] = base.getProject(a['project'])
				#a['city'] = base.getCity(a['city'])
				a['idc'] = base.getIDC(a['idc'])
				#a['status'] = base.getStatus(a['status'])
				newapplys.append(a)

			#page Nav
			if applycount <= pagesize:
				page_nav = u"共 <b>%s</b> 条记录" % (applycount)
			elif page == 1:
				page_nav = u"<a href='/main?page=%d'>下一页</a> | <a href='/main?page=%d'>尾页</a> | (%d / %d) , 共 <b>%d</b> 条记录" % (page+1,pages,page,pages,applycount)
			elif page == pages:
				page_nav = u"<a href='/main?page=%d'>上一页</a> | <a href='/main?page=1'>首页</a> | (%d / %d) , 共 <b>%d</b> 条记录" % (page-1, page, pages,applycount)
			else:
				page_nav = u"<a href='/main?page=%d'>下一页</a> | <a href='/main?page=%d'>上一页</a> | <a href='/main?page=1'>首页</a> | <a href='/main?page=%d'>尾页</a> | (%d / %d) , 共 <b>%d</b> 条记录" % (page+1,page-1,pages,page,pages,hostcount)
				
			if web.ctx.session.privilege == 0:
				return render.application(newapplys,pages,page,page_nav)
			else:
				return render.application_admin(newapplys,pages,page,page_nav)
...
class AddHost:
	...
	def POST(self):
		...
		for ip in addr1_ip,addr1_gateway,addr2_ip,addr2_gateway:
			if ip == '':
					return base.code("<script language='javascript'>alert('IP地址不能为空!');window.history.back(-1);</script>")
			if not base.checkIp(ip):
					return base.code("<script language='javascript'>alert('IP地址不合法，请检查 !');window.history.back(-1);</script>")
		...
		if result == 'addr':
			return base.code("<script language='javascript'>alert('该IP地址已经存在!');window.history.back(-1);</script>")
		elif result == 'hostname':
			return base.code("<script language='javascript'>alert('该主机标识已经存在!');window.history.back(-1);</script>")
		else:
...
class AddApply:
	def GET(self):
		...
		if web.ctx.session.privilege == 0:
			...
			return render.add_apply(project, idc)

	def POST(self):
		sitename = web.input().sitename.strip()
		idc = web.input().idc
		project = web.input().project
		addr_ip = web.input().machineip.strip()
		sofeware = web.input().sofeware.strip()
		datadir = web.input().datadir.strip()
		startscrite = web.input().startscrite.strip()
		port = web.input().port.strip()
		mysql_conf = web.input().mysql_conf.strip()
		mysql_ip = web.input().mysql_ip.strip()
		mysql_stype = web.input().mysql_stype.strip()
		mysql_name = web.input().mysql_name.strip()
		mysql_use = web.input().mysql_use.strip()
		mysql_pw = web.input().mysql_pw.strip()
		dev_name = web.input().dev_name.strip()
		mysql_stype = web.input().mysql_stype.strip()
		mysql_name = web.input().mysql_name.strip()
		other = web.input().other
		...
		# Input check
		if sitename == '':
			return base.code("<script language='javascript'>alert('站点名称必须输入 !');window.history.back(-1);</script>")
		#if port != '':	
		#	if not base.checkPort(port):
		#		return base.code("<script language='javascript'>alert('端口不合法，请检查 !');window.history.back(-1);</script>")
		...
		#			return base.code("<script language='javascript'>alert('CPU/MEMORY/DISK不合法，请检查 !');window.history.back(-1);</script>")
		if addr_ip == '':
			return base.code("<script language='javascript'>alert('服务器IP必须输入 !');window.history.back(-1);</script>")
	 	if addr_ip != '':
				if not base.checkIp(addr_ip):
					return base.code("<script language='javascript'>alert('IP地址不合法，请检查 !');window.history.back(-1);</script>")

		# Save host to db
		result = models.insert_apply(sitename,idc,project,addr_ip,sofeware,datadir,startscrite,port,mysql_conf,mysql_ip,mysql_stype,mysql_name,mysql_use,mysql_pw,dev_name,other,int(time.time()),'',web.ctx.session.username)
		if result:
			raise web.seeother('/apply')
		else:
			return base.code("<script language='javascript'>alert('此应用已在该服务器上部署，请检查 !');window.history.back(-1);</script>")
...
class ModifyApplys:
	def GET(self):
		...
		if action == 'ma':
			...
			idc = models.getIDCInfo()
			#status = models.getStatusInfo()
			return render.modify_apply(project,idc,apply[0])
		elif action == 'va':
			#city = models.getCityInfo()
			column = web.input().column
			project = models.getProjectInfo()
			idc = models.getIDCInfo()
			#status = models.getStatusInfo()
			apply_list = list(apply)
			t = apply_list[0]['modifytime']
			ltime = time.localtime(int(t))
			modifytime = time.strftime("%Y-%m-%d %H:%M:%S",ltime)
			apply_list[0]['modifytime'] = modifytime
			if column == 'sitename':
				return render.view_apply(project,idc,apply_list[0])
				#raise web.seeother("/mhosts/?action=vh&hid=34",True)
			if column == 'addr_ip':
				aid = web.input().aid
				#raise web.seeother('/index')
				#ip = web.input().machineip
				#hid = web.input().hid
				hid = models.getHostHidByApply_addr(aid)
				host = models.getOneHost(hid)
				city = models.getCityInfo()
				project = models.getProjectInfo()
				idc = models.getIDCInfo()
			...
			# Save to history
			a = models.getOneApply(aid)[0]
			content = "INSERT INTO applications VALUES (%s,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (a['aid'],a['idc'],a['sitename'],a['project'],a['addr_ip'],a['sofeware'],a['datadir'],a['startscrite'],a['port'],a['mysql_conf'],a['mysql_ip'],a['mysql_stype'],a['mysql_name'],a['mysql_use'],a['mysql_pw'],a['dev_name'],a['other'],a['jointime'],a['modifytime'],a['modifyman'])
			description = u"<b>站点名称：</b>%s<br><b>服务器IP：</b>%s" % (a['sitename'],a['addr_ip'])
			delman = web.ctx.session.username
			deltime = int(time.time())
			models.addHistory(content,description,delman,deltime)
	...
	def POST(self):
		...
		result = models.modifyApply(aid,sitename,idc,project,addr_ip,sofeware,datadir,startscrite,port,mysql_conf,mysql_ip,mysql_stype,mysql_name,mysql_use,mysql_pw,dev_name,other,int(time.time()),web.ctx.session.username)
		if result:
			raise web.seeother('/apply')
		else:
			return base.code("<script language='javascript'>alert('失败，该应用已存在!');window.history.back(-1);</script>")
...
class Sysview:
	def GET(self):
		...
		return render.sysview(conf,sys_overview,status_overview)
	...
	def POST(self):
		...
		if input.name == '':
			...
			raise web.seeother('/sysview')

class Index:
	def GET(self):
		return render_plain.index()
        #return render_plain.index2()
